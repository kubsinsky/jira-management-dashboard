...
(function (global) {
  ...
  function onIssueTypeChange(val) {
    const epicFt    = getCachedEl('ci-epic-fasttrack');
    const epicName  = getCachedEl('ci-epicname-field');
    const parentRow = getCachedEl('ci-parent-row');
  ...
  function _renderCiPriorityItems() {
    ...
    const fallbackPriorities = [
      { name: 'Highest', label: 'Highest', cls: 'priority-highest', dot: '↑' },
      { name: 'High', label: 'High', cls: 'priority-high', dot: '↑' },
      { name: 'Medium', label: 'Medium', cls: 'priority-medium', dot: '●' },
      { name: 'Low', label: 'Low', cls: 'priority-low', dot: '↓' },
      { name: 'Lowest', label: 'Lowest', cls: 'priority-lowest', dot: '↓' }
    ];
    const prios = deps.PRIORITIES || fallbackPriorities;
    listEl.innerHTML = prios.map(p => `
      <div class="status-inline-item" onclick="selectCiPriority('${deps.escHtml(p.name)}','${p.dot || ""} ${deps.escHtml(p.label || p.name)}')" style="padding:6px 14px; cursor:pointer;">
        <span class="badge ${p.cls || ""}">${p.dot || ""} ${p.label || p.name}</span>
      </div>`).join('');
  }
  ...
  function _renderCiIssueTypeItems() {
    ...
    if (!listEl) return;
    const fallbackTypes = [{name: 'Task'}, {name: 'Story'}, {name: 'Bug'}, {name: 'Epic'}, {name: 'Subtask'}];
    const types = deps.ISSUE_TYPES || fallbackTypes;
    listEl.innerHTML = types.map(it => {
      const name = typeof it === 'string' ? it : it.name;
      const icon = typeof it === 'string' ? '' : (it.iconHtml || '');
      return `
      <div class="status-inline-item" onclick="selectCiIssueType('${deps.escHtml(name)}')" style="padding:7px 14px; cursor:pointer; font-weight:500;">
        ${icon} ${deps.escHtml(name)}
      </div>`;
    }).join('');
  }
  ...
  async function _renderCiSprintItems() {
    ...
    const boardId = deps.BOARD_ID || (deps.CFG && deps.CFG.boardId);
    if (boardId) {
      try {
        const r = await fetch(`/agile/board/${boardId}/sprint?state=active,future&maxResults=20`);
        if (r.ok) {
